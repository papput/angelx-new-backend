...
# Complete Transaction Flow Testing Script for AngelX
# Tests the entire deposit and withdrawal flow including wallet management

echo "=== AngelX Complete Transaction Flow Testing ==="
echo
...
if echo "$response" | grep -q "success.*true"; then
    ...
    deposit_count=$(echo "$response" | grep -o '"deposit":\[[^][]*\]' | grep -o '{' | wc -l | tr -d ' ')
...
withdrawal_data="{\"walletId\":\"$USER_WALLET_ID\",\"amount\":500}"
...
echo "2. Admin created deposit method"
